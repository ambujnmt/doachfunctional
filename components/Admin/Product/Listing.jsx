...
import { productList } from "../../../utils/fetchAdminApi";
...
export default function Listing() {
  ...
  const handleDelete = async (id) => {
    await confirmDelete(`/delete/product/${id}`, fetchProducts);
  };
        ...
        <div className="mb-6 flex justify-between items-center">
          <div>
            ...
            <p className="text-gray-400 mt-1">
              Manage and view product list here.
            </p>
          </div>
          <div className="flex space-x-2">
            ...
            {/* Create Button */}
            <Link
              href="/administor/product/create"
            ...
            <div>
                  ...
                  {currentProducts.length > 0 ? (
                    currentProducts.map((product, index) => (
                      ...
                      >
                        ...
                        <td className="p-3">
                            {product.images && product.images.find(img => img.type === "thumbnail") ? (
                                <img
                                src={product.images.find(img => img.type === "thumbnail").image_url}
                        ...
                        <td className="p-3 font-medium text-white">{product.product_name}</td>
                        <td className="p-3 text-gray-300">${product.price}</td>
                        <td className="p-3 text-gray-300">{product.stock}</td>
                        <td className="p-3">
                          <span
                        ...
                        <td className="p-3 text-center">
                          <div className="flex justify-center space-x-2">
                            <Link
                              href={`/administor/product/edit?id=${product.id}`}
              ...
              <div className="p-4 flex justify-between items-center border-t border-gray-700">
                <span className="text-gray-400 text-sm">
                  Showing {indexOfFirst + 1} to {Math.min(indexOfLast, filteredProducts.length)} of {filteredProducts.length} entries
                </span>
                {totalPages > 1 && (
                  <Pagination
                    count={totalPages}
