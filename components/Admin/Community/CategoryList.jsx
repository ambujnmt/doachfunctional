...
import { categoryCommunityList } from "../../../utils/fetchAdminApi";
...
export default function CategoryList() {
  ...
  const fetchCategories = async () => {
    try {
      const data = await categoryCommunityList();
      setCategories(data || []);
    } catch (error) {
  ...
  const handleDelete = async (id) => {
    await confirmDelete(`/delete/communityCategory/${id}`, fetchCategories);
  };
  ...
  const filteredCategories = categories.filter(
    (cat) =>
      ...
      (cat.category_date &&
        new Date(cat.category_date).toLocaleDateString().includes(searchTerm))
  );
        ...
        <div className="mb-6 flex justify-between items-center">
          <div>
            ...
            <p className="text-gray-400 mt-1">
              Manage and view category details here.
            </p>
          </div>
          <div className="flex space-x-2">
            ...
            <Link
              href="/administor/community/categoryCreate"
              className="px-4 py-2 rounded-lg bg-yellow-500 text-black font-semibold hover:bg-yellow-600 transition inline-block"
            >
              + Create
            </Link>
                ...
                <tbody>
                    {currentCategories.length > 0 ? (
                        ...
                        >
                            ...
                            <td className="p-3 text-gray-300">{cat.description}</td>
                            <td className="p-3">
                            <span
                            ...
                            <div className="flex justify-center space-x-2">
                                <Link
                                href={`/administor/community/categoryEdit?id=${cat.id}`}
